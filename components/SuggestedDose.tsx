...
export function SuggestedDose({ result, loadingDose, target }: Props) {
  return (
      ...
      {result ? (
        <>
          ...
          <div className="rounded-lg bg-ink-50 dark:bg-ink-950/50 border border-ink-200 dark:border-ink-800 p-3 space-y-1.5">
            <Row label="AUC₂₄" value={`${fmt.auc(result.auc24)} mcg·h/mL`} inRange={result.auc24 >= target.aucMin && result.auc24 <= target.aucMax} />
          ...
          {loadingDose !== null && (
            <div className="mt-3 flex items-start gap-2 rounded-lg border border-accent-500/30 bg-accent-500/5 p-3">
              <div className="text-xs">
                <div className="font-semibold text-accent-600 dark:text-accent-400">Loading dose</div>
                <div className="text-ink-700 dark:text-ink-200 mt-0.5">
                  {fmt.mg(loadingDose)} once, then start maintenance after one interval.
                </div>
              </div>
            </div>
          )}
...
function Stat({ label, value, big }: { label: string; value: string; big?: boolean }) {
  return (
    <div className="rounded-lg border border-ink-200 dark:border-ink-800 bg-white dark:bg-ink-900 p-3">
      ...
      <div className={big ? "text-xl font-bold tabular-nums text-ink-900 dark:text-ink-50 mt-1" : "stat-value mt-0.5"}>
        {value}
      </div>
    </div>
  );
...
function Row({ label, value, inRange }: { label: string; value: string; inRange?: boolean }) {
  return (
    <div className="flex items-baseline justify-between text-sm">
      <span className="text-ink-500 dark:text-ink-400">{label}</span>
      <span className={`font-medium tabular-nums ${inRange === undefined ? "text-ink-900 dark:text-ink-100" : inRange ? "text-accent-600 dark:text-accent-400" : "text-amber-600 dark:text-amber-400"}`}>
        {value}
      </span>
    </div>
  );
